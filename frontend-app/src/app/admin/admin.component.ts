    ...
    <div style="max-width:1100px;margin:0 auto;padding:20px 16px 60px;">
      ...
      <div style="background:linear-gradient(135deg,#0d3320,#1a5c38 60%,#236b45);border-radius:22px;padding:26px;margin-bottom:22px;box-shadow:0 16px 48px rgba(15,50,30,0.2);position:relative;overflow:hidden;">
        ...
        <div style="position:relative;z-index:1;">
          ...
          <p style="color:rgba(255,255,255,0.6);font-size:0.82rem;margin:0 0 22px;">El sistema detecta plantas, cantidades y costos para reducir entrada manual y mantener el catálogo actualizado.</p>
          ...
          <div *ngIf="invoiceResult?.items?.length" style="background:white;border-radius:14px;padding:18px;margin-top:14px;">
            <h3 style="margin:0 0 12px;color:#102319;font-size:0.95rem;font-weight:700;">Plantas detectadas</h3>
            <div *ngFor="let item of invoiceResult!.items; let i = index" style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:9px;">
              <input type="text" [(ngModel)]="item.plant_name" class="form-input" style="flex:1 1 100%;" placeholder="Nombre">
              <input type="number" [(ngModel)]="item.quantity" class="form-input" style="flex:1;min-width:70px;" placeholder="Cantidad">
              <input type="number" step="0.01" [(ngModel)]="item.unit_cost" class="form-input" style="flex:1;min-width:70px;" placeholder="Costo">
              <button (click)="removeItemFromInvoice(i)" style="background:#fff0f0;border:none;color:#9b1c1c;border-radius:10px;padding:10px 14px;font-weight:700;cursor:pointer;">✕</button>
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px;">
              <button (click)="confirmRestock()" class="btn-primary" style="flex:1;min-width:130px;">✅ Confirmar actualización</button>
