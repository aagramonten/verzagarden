...
@Component({
  ...
  styles: [`
    ...
    .plant-name { margin:0 0 3px 0; font-size:1.05rem; font-weight:700; color:#102319; line-height:1.2; }
    .plant-price { font-size:1rem; font-weight:700; color:#1f7a4d; margin-bottom:7px; }
    .plant-desc { font-size:0.8rem; color:#516052; margin:0 0 10px 0; line-height:1.4;
      display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
    .care-row { display:flex; gap:6px; flex-wrap:wrap; margin-bottom:12px; }
    .chip { background:#f4f8f1; border:1px solid #eef1ec; padding:4px 10px;
      border-radius:20px; font-size:0.72rem; color:#516052; display:flex; align-items:center; gap:4px; }
    .wa-btn {
    ...
    .lightbox-close {
      ...
      width:42px; height:42px; border-radius:99px; font-size:1.2rem;
    ...
    .admin-badge { font-size:0.68rem; font-weight:700; padding:3px 8px; border-radius:8px; }
    .btn-edit { background:#f4f8f1; border:1px solid #dfe7dd; color:#102319; border-radius:9px; padding:7px 14px; font-weight:600; cursor:pointer; font-size:0.82rem; }
    .btn-delete { background:#fff0f0; border:1px solid #fad5d5; color:#9b1c1c; border-radius:9px; padding:7px 14px; font-weight:600; cursor:pointer; font-size:0.82rem; }
  `],
  template: `
    <ng-container *ngIf="!adminMode">
      <article class="plant-card">
        <div class="card-img-wrap" (click)="plant.image_url && openLightbox()">
        ...
        <div class="card-body">
          ...
          <p class="plant-desc">{{ plant.description || 'Planta resistente y fácil de cuidar.' }}</p>
          <div class="care-row">
            <span class="chip">☀️ {{ plant.light || 'Luz indirecta' }}</span>
            <span class="chip">💧 {{ plant.water || 'Riego moderado' }}</span>
          </div>
    ...
    <ng-container *ngIf="adminMode">
      <div class="admin-row">
        <div class="admin-thumb">
          ...
          <div *ngIf="!plant.image_url" style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:1.4rem;">🪴</div>
        </div>
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:0.78rem;color:#516052;margin-top:2px;">{{ plant.category }} · \${{ plant.price }}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex-shrink:0;">
          <div style="font-weight:700;color:#102319;font-size:0.88rem;">{{ plant.stock }} u.</div>
